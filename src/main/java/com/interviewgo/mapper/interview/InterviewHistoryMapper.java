...
public interface InterviewHistoryMapper {
	...
	InterviewHistoryDTO selectLastInterviewHistoryBySsid(@Param("iv_ssid") String uuid, @Param("mb_uid") Long mbUid);
	
	// 채팅 가져오는 용도로만 사용 (새로고침시 데이터 삭제 방지)
	List<InterviewHistoryDTO> selectAllInterviewHistoryBySsid(@Param("iv_ssid") String uuid, @Param("mb_uid") Long mbUid);
	
	// 면접 기록 삽입 (단일 채팅, AI 반환값)
	int insertInterviewHistory(InterviewHistoryDTO interviewHistory);	
	
	// 세션 Id, mb_uid 기반 면접기록 삭제
	int deleteInterviewHistoryBySsid(@Param("iv_ssid") String uuid, @Param("mb_uid") Long mbUid);
